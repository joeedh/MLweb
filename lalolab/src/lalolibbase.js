...
////////////////////////////////////
export var printPrecision = 3; // number of digits to print
...
export function size( A, sizealongdimension ) {
  ...
  switch( type(A) ) {
    ...
    case "matrix":
      ...
      break;
    default:
      window.LALOLIB_ERROR = "Cannot determine size of object";
      return undefined;
      break;
  }
...
export function laloprint(x, htmlId, append) {
  /*
...
export function printMat(A, size, htmlId, append) {
  if (typeof (append) === "undefined")
    append = false;
...
// Plots
//////////
export function plot(multiargs) {
  ...
  var plotinfo = {
    "data": data, "minX": minX, "maxX": maxX, "minY": minY, "maxY": maxY, "styles": styles, "legend": legends
  };
...
// Color plot
export function colorplot(multiargs) {
  ...
  var plotinfo = {
    "x": x, "y": y, "z": z, "minX": minX, "maxX": maxX, "minY": minY, "maxY": maxY, "minZ": minZ, "maxZ": maxZ
  };
...
// 3D plot
export function plot3(multiargs) {
...
export function mousezoom(e, delta, plotidx) {
  if (!e)
    e = window.event;
...
export function zoomoriginal(plotidx) {
  LALOLibPlots[plotidx].resetzoom();
}

export var LALOLABPLOTxprev = 0;
export var LALOLABPLOTyprev = 0;

export function mouseposition(e, plotidx) {
  var plotcanvas = document.getElementById(LALOLibPlots[plotidx].canvasId);
  var rect = plotcanvas.getBoundingClientRect();
...
export function mousestartmove(e, plotidx) {
  if (e.button === 0) {
...
export function mousestopmove(e) {
  LALOLABPLOTMOVING = false;
}

export function mouseimageposition(e, plotidx) {
  var plotcanvas = document.getElementById(LALOLibPlots[plotidx].canvasId);
  var rect = plotcanvas.getBoundingClientRect();
...
export function laloparse(WorkerCommand) {
  // Parse Commands
...
export function parseSplittedCommand(cmd) {
  //console.log("parsing : " + cmd);
  // !!! XXX should parse unary ops before all the others !!!

  var ops = ["==", "!=", ">=", "<=", ">", "<", "\\", ":", "+", "-", ".*", "*", "./", "^", "'"]; // from lowest priority to highest
  var opsFcts = ["isEqual", "isNotEqual", "isGreaterOrEqual", "isLowerOrEqual", "isGreater", "isLower", "solve",
                 "range", "add", "sub", "entrywisemul", "mul", "entrywisediv", "pow", "undefined"];
...
export function parseAssignment(assignmentStr) {
...
export function parseBrackets(cmdString) {
...
export function parseRangeRange(cmd) {
...
/// Lab 
////////////////////////////
export function MLlab(id, path) {
  var that = new Lalolab(id, true, path);
  return that;
}

export class Lalolab {
  constructor(id, mllab, path) {
...
export function importLaloScript(script) {
...
export function getObjectWithoutFunc(obj) {
...
export function renewObject(obj) {
  ...
  switch (to) {
    ...
    default:
      ...
      if (typearray[1] === "SVM" || typearray[1] === "SVR") {
        newobj["kernelFunc"] = kernelFunction(newobj["kernel"], newobj["kernelpar"], type(newobj["SV"]) === "spmatrix"
                                                                                     ? "spvector" : "vector");
      }
      if (typearray[1] === "KernelRidgeRegression") {
        newobj["kernelFunc"] = kernelFunction(newobj["kernel"], newobj["kernelpar"], type(newobj["X"]) === "spmatrix"
                                                                                     ? "spvector" : "vector");
      }
...
export function load_data(datastring) {
...
//// progress /////////////////////
export function notifyProgress(ratio) {
